import "../backlink.css";

export default function DarkHorsePage() {
  return (
    <>
      <div
        dangerouslySetInnerHTML={{
          __html: `<!-- "Think about the numbers you see or you don't see." -->`,
        }}
      />

      <main className="creepy-page">
        <section className="creepy-card">
          <h1 className="creepy-title">ARCHIVED DRAFT</h1>

          <div className="creepy-list">
            <div className="creepy-row">Three witnesses entered.</div>
            <div className="creepy-row">Their stories became one.</div>
            <div className="creepy-row">Numbers in the video?</div>
            <div className="creepy-row">Add it together.</div>
            <div className="creepy-row">One witness lied.</div>
            <div className="creepy-row">Remove him.</div>
            <div className="creepy-row">The truth stands alone.</div>
            <div className="creepy-row">Square it.</div>
            <div className="creepy-row">Then think internationally.</div>
          </div>

          <p className="creepy-note">
            <a
              href="https://youtu.be/-Ru39FMzIvs"
              target="_blank"
              rel="noopener noreferrer"
              style={{ color: "inherit", textDecoration: "underline" }}
            >
              https://youtu.be/-Ru39FMzIvs (8:28)
            </a>
          </p>
        </section>
      </main>
    </>
  );
}